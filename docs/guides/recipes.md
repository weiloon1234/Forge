...
Copy-pasteable paths for common app work. The examples assume the split bootstrap layout used by Forge-starter, but the same APIs work in smaller apps.
...
For large exports, return `DatatableExportAccepted` and let Forge queue the XLSX generation job.
