...
pub fn expand(input: DeriveInput) -> syn::Result<TokenStream> {
    if !input.generics.params.is_empty() {
        return Err(syn::Error::new_spanned(
            &input.generics,
            "ForgeId does not support generic enums",
        ));
    }
    ...
    let id_path = args
        .id
        .ok_or_else(|| syn::Error::new_spanned(&ident, "missing #[forge(id = ...)] attribute"))?;
    ...
    let Data::Enum(data) = input.data else {
        return Err(syn::Error::new_spanned(
            ident,
            "ForgeId can only be derived for enums",
        ));
    };
    ...
    for variant in data.variants {
        if !matches!(variant.fields, Fields::Unit) {
            return Err(syn::Error::new_spanned(
                variant,
                "ForgeId can only be derived for fieldless enum variants",
            ));
        }
        ...
        let value = match variant_args.value {
            ...
            None => {
                return Err(syn::Error::new_spanned(
                    variant.ident,
                    "missing #[forge(value = \"...\")] variant attribute; add rename_all = \"snake_case\" to the enum to derive values automatically",
                ))
            }
        };

        if !seen_values.insert(value.value()) {
            return Err(syn::Error::new_spanned(value, "duplicate ForgeId value"));
        }
...
fn parse_enum_args(attrs: &[syn::Attribute]) -> syn::Result<ForgeIdArgs> {
    let mut args = ForgeIdArgs::default();
        ...
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident("id") {
                ...
                return Err(meta.error(
                    "unsupported forge attribute for ForgeId derive; expected id = ... or rename_all = \"snake_case\"",
                ));
            }
            Ok(())
        })?;
    ...
    for attr in attrs.iter().filter(|attr| attr.path().is_ident("forge")) {
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident("value") {
                ...
                return Err(meta.error(
                    "unsupported forge variant attribute for ForgeId derive; expected value = \"...\"",
                ));
            }
            Ok(())
        })?;
